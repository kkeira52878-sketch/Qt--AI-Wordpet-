...
class VocabularyWindow(QWidget):
    ...
    def _setup_ui(self):
        ...
        notebook_label.setStyleSheet(
            "font-size: 13px; font-weight: bold; color: #4A3FB5;"
        )
    ...
    def _load_words(self, filter_text: str = ""):
        ...
        if filter_text:
            ...
            words = [
                w for w in words if ft in w[1].lower() or (w[3] and ft in w[3].lower())
            ]
    ...
    def _on_item_clicked(self, item: QListWidgetItem):
        ...
        example = word[4] or "-"
        mastery = word[5] or 0
        notes = word[11] if len(word) > 11 else ""

        dialog = QDialog(self)
        dialog.setWindowTitle(f"单词详情 — {word[1]}")

        # ----------修改3：参数380->420,新增QTextEdit
        ...
        notes_edit.setPlaceholderText("Your ideas")
        ...
        save_btn.clicked.connect(
            lambda: self._save_word_notes(word[1], notes_edit.toPlainText(), dialog)
        )
